...
int main(void) {
    ...
    if (child2 == 0) {
        print_info("child 2 before exec");
        printf("Child 2 will be replaced by lab7_threads using execl()\n");
        fflush(stdout);
        execl("./lab7_threads", "lab7_threads", NULL);
        perror("execl");
        return 2;
    }
    ...
    system("ps -x | grep -E 'lab7_exec|lab7_threads|PID|ps' | grep -v grep");

    int status;
    waitpid(child1, &status, 0);
    printf("Parent: child1 finished, status=%d\n", status);
    waitpid(child2, &status, 0);
    printf("Parent: child2 finished after exec, status=%d\n", status);
    return 0;
}
